    ...
    <script>
        ...
        function create() {
            ...
            this.raycaster = this.raycasterPlugin.createRaycaster({
                ...
                mapSegmentCount: 32 // Reduce the number of segments for better performance
            });
            this.ray1 = this.raycaster.createRay({
                ...
                angle: 0,
                collisionRange: 200 // Limit the range to reduce calculations
            });
            ...
            this.overlay.fillRect(0, 0, levelWidth * tileSize, levelHeight * tileSize);
        ...
        function update() {
            ...
            if (this.frameCounter % 5 === 0) {
                const intersections = [];
                for (let angle = -15; angle < 360; angle += 15) { // Adjust the step to balance performance and accuracy
                    this.ray1.setAngle(Phaser.Math.DegToRad(angle));
                    this.ray2.setAngle(Phaser.Math.DegToRad(angle + 5)); // Offset the second ray by 10 degrees
                ...
                // Create a hole in the overlay for the viewing cone
                this.overlay.fillStyle(0x000000, 0);
                this.overlay.beginPath();
                this.overlay.moveTo(this.player.x, this.player.y);
                intersections.forEach(intersection => {
                    this.overlay.lineTo(intersection.x, intersection.y);
                });
                this.overlay.closePath();
                this.overlay.fillPath();

                // Redraw the player and walls within the viewing cone
                this.player.setVisible(true);
                this.walls.getChildren().forEach(wall => {
                    const wallBounds = wall.getBounds();
                    const isVisible = intersections.some(intersection => Phaser.Geom.Rectangle.ContainsPoint(wallBounds, intersection));
                    wall.setVisible(isVisible);
                });
            }
        ...
        function addFeature(scene, feature, x, y) {
            switch (feature) {
                ...
                case 'trap':
                    ...
                    scene.add.rectangle(x * tileSize, y * tileSize, tileSize, tileSize, 0x8b0000).setOrigin(0); // Dark red color for trap
                    break;
            }
        }
